"""
batch_run_10cases.py
====================
Run the full HU pipeline + analysis on all 10 patient cases.

Cases: L067, L096, L109, L143, L192, L286, L291, L310, L333, L506
"""
...
def main():
    ...
    print("\n\n" + "=" * 72)
    print(f"  BATCH SUMMARY — {len(CASES)} CASES")
    print("=" * 72)
    ...
    for case in CASES:
        ...
        if r["status"] == "OK":
            ...
            total_pipeline += r['pipeline_time']
            print(f"  {case:<8} {'OK':<10} {r['score']:>5.0f}% {organs:>8} "
                  f"{pt:>10} {at:>10}")
        elif r["status"] == "FAILED":
            fail_count += 1
            print(f"  {case:<8} {'FAILED':<10}   —  error: {r['error'][:50]}")
        else:
            skip_count += 1
            print(f"  {case:<8} {'SKIPPED':<10}")

    print()
    print(f"  Passed: {ok_count}/{len(CASES)}  |  "
          f"Failed: {fail_count}  |  Skipped: {skip_count}")
    if ok_count:
        print(f"  Avg pipeline time: {total_pipeline/ok_count:.0f}s per case")
    print()
